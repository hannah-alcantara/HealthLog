...
Users want to record past doctor visits including date, doctor name, reason for visit, and notes from the appointment. This creates a searchable history of their healthcare interactions.

**Why this priority**: Appointment history builds on the medical history foundation and provides historical context. It's valuable but users can get initial value from P1 alone.

**Independent Test**: Can be tested by creating appointment records with visit details and doctor notes, then viewing the chronological history. Success means users can maintain a complete log of their medical appointments.
...
- **FR-006**: System MUST allow users to add appointment records with date, doctor name, reason for visit, and notes
- **FR-007**: System MUST allow users to edit appointment records including adding/modifying doctor notes
...
- **Appointment**: Represents a past medical visit with date, doctor name, reason for visit, notes from the appointment; creates historical record of healthcare interactions
